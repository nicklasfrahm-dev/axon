...
import { AxonClient, Service } from "./types";

const ROOT = path.resolve(process.cwd(), "../"); // repo root from axon/ folder
...
export class FilesystemClient implements AxonClient {
  ...
  async listTenants(): Promise<string[]> {
    const clustersDir = path.join(ROOT, "deploy/clusters");
    const clusters = readDirSafe(clustersDir).filter((f) =>
      fs.statSync(path.join(clustersDir, f)).isDirectory()
    );
    const tenants: string[] = [];
    for (const c of clusters) {
      const clusterDir = path.join(clustersDir, c);
      const entries = readDirSafe(clusterDir).filter((f) =>
        fs.statSync(path.join(clusterDir, f)).isDirectory()
      );
      for (const t of entries) tenants.push(t);
    }
